...
for(o in phecode_key$varname){
  for(s in scorelist){
    for(a in names(adjustmentList)){
      ...
      # for the diabetes outcome, remove those with prevalent diabetes
      # should now be superfluous with the above statement
      # if(o=="X250.2"){dataForModelsSubset <- dataForModelsSubset %>% filter(newDiabetes!="Yes")}
...
for(event in setdiff(levels(fineGrayData$fg.event), c("CENSOR", "OTHER_DEATH"))){
  for(a in names(adjustmentList)){
    ...
    # ## coxph approach
    # 
    # fg.dat <- finegray(fmla,
    #                    data = fineGrayData,
    #                    etype = event)
    # 
    # # update fmla for coxph
    # fmla <- as.formula(paste("Surv(fgstart, fgstop, fgstatus) ~", rhs))
    # fg.fit <- coxph(fmla,
    #                  data=fg.dat,
    #                  weight= fgwt)
    # 
    # fg.fit.sum <- summary(fg.fit)
    # print(fg.fit)
